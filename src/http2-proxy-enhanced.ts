...
import type { ClientHttp2Session, OutgoingHttpHeaders, IncomingHttpHeaders } from 'http2'
...
class Http2ConnectionPool {
  ...
  getSession(origin: string, options: NormalizedProxyOptions): ClientHttp2Session {
...
function createHttp2Headers(
  ...
  const forbiddenHeaders = ['connection', 'upgrade', 'keep-alive', 'transfer-encoding', 'proxy-connection']
  ...
  for (const [key, value] of Object.entries(req.headers)) {
    ...
    if (options.preserveHeaderKeyCase) {
      ...
      const originalKey = Object.keys(req.headers).find(k => k.toLowerCase() === key)
...
async function proxyHttp2Request(
  ...
  stream.on('error', (err) => {
    ...
    logProxyRequest(logger, req.method || 'GET', req.url || '', targetUrl.href, 502, duration)
    if (!res.headersSent) {
  ...
  stream.on('response', (responseHeaders: IncomingHttpHeaders) => {
    ...
    logProxyRequest(logger, req.method || 'GET', req.url || '', targetUrl.href, status, duration)
    ...
    if (options.followRedirects && status >= 300 && status < 400) {
      ...
      if (location) {
        ...
        const rewrittenLocation = locationUrl.href.replace(targetUrl.origin, '')
    ...
    for (const [key, value] of Object.entries(responseHeaders)) {
      if (!key.startsWith(':')) {
        ...
        if (key === 'set-cookie' && (options.cookieDomainRewrite || options.cookiePathRewrite)) {
    ...
    if (options.sse && cleanHeaders['content-type']?.includes('text/event-stream')) {
