...
export function createLogger(namespace: string, viteConfig?: ResolvedConfig): Logger {
  const debug = debuglog(namespace)
  const viteLogger = viteConfig?.logger

  return {
    debug: (msg: string, ...args: any[]) => {
      if (process.env.DEBUG?.includes(namespace) || process.env.LOG_LEVEL === 'debug') {
        debug(msg, ...args)
      }
    },
    ...
    error: (msg: string, ...args: any[]) => {
      if (viteLogger) {
        viteLogger.error(`[${namespace}] ${msg}`, { timestamp: true, error: args[0] })
      } else {
