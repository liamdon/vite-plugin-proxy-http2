    ...
    async configureServer(server) {
      ...
      if (!existsSync(keyPath) || !existsSync(certPath)) {
        ...
        await new Promise((resolve, reject) => {
          const certProcess = spawn('node', [join(__dirname, 'generate-certs.js')], {
            stdio: 'inherit'
          })
      ...
      if (!portAvailable) {
        console.error('❌ Port 9443 is already in use. Please stop any process using that port.')
        process.exit(1)
      }
      ...
      serverProcess.on('exit', (code, signal) => {
        if (!isShuttingDown) {
          console.error(`[HTTP/2 Server] Exited unexpectedly with code ${code} and signal ${signal}`)
        }
        serverProcess = null
      })
